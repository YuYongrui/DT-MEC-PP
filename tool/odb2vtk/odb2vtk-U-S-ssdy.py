...
def ConvertOdb2Vtk(filename = 'C:\Temp\odb2vtk.txt'):  #Modify the default value of filename here to specify the default configuration file
	...
	for i in range(death_num):
		death_ele.append(64056 + i)
		death_ele.append(64684 + i)
		...
		for ins_i in input_instance:
			...
			for i_frame in input_frame:
				...
				for pn in range(piecenum):
					...
					outfile.write("<"+"PointData"+" "+"Vevtors="+'"'+"Spatial_displacement"+'"'\
					...
					if (mesh_type == 12):
						for i in range(len(stg_e)/8):
							# i represent element index, when i equals death element's index, write "0 0 0 0 0 0 0 0"
							if(i in death_ele):
								print "0000"
				...
				if ( piecenum > 1 ):
					...
					#pointdata
					outfile.write("<"+"PPointData"+" "+"Vevtors="+'"'+"Spatial_displacement"+'"'\
					+" "+"Scalars="+'"'+"Stress_Mises"+'"'+">"+'\n')
